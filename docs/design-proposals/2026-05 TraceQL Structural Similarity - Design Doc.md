...
# Design Doc: Baseline Trace Selection via Operation Set Similarity

## 1. Background

Trace comparison is a core debugging workflow: given a slow or broken trace, find a healthy trace with the same shape from a different time window and diff them to understand what changed.
...
This proposal introduces a Tempo-native trace shape similarity primitive that solves the baseline selection problem at the storage level, eliminating client-side complexity and enabling a single TraceQL query to find baseline candidates.

### The N×N problem

Without trace shape filtering, finding a baseline requires:
...
3. Compute operation set similarity against the seed trace → N comparisons
...
| **No storage-level filtering** | All traces matching the selector are returned; shape filtering happens after the fact |
| **Duplicated logic** | Every consumer must reimplement the same orchestration and heuristics |
| **Not composable** | Cannot combine trace shape similarity with TraceQL metrics (`quantile_over_time`, `rate`) |
...
Keep baseline selection outside Tempo. Each consumer (Grafana plugin, CLI, agent) implements the full workflow: fetch seed trace → extract attributes → build TraceQL search → fetch candidates → score by operation overlap → pick best match.
...
### Option B: MinHash-based operation set similarity in Tempo (proposed)

Compute a MinHash signature for each trace at block build time. Store it as 4 uint64 columns in the existing parquet schema. Expose a `similar_to()` predicate in TraceQL that filters traces by operation set similarity at the storage level.
...
Two traces have the same shape when they involve the same set of services and operations — regardless of latency, errors, or timing. A checkout trace that calls `payment → inventory → shipping` has a different shape than one that calls `payment → fraud-check → inventory → shipping`, even if both are from the same endpoint.

Importantly, this compares the **set of operations**, not the call graph topology. Parent-child relationships, call depth, ordering, and span counts are not considered. Two traces that call the same operations in completely different tree arrangements are treated as the same shape.
...
- J = 1.0 → identical shape (same services, same operations)
...
These are deliberate trade-offs. The signature answers "what services and operations are involved?" not "how are they arranged?" The diff engine handles the call graph details after the baseline is selected.
...
Returns the p50 duration of shape-similar healthy traces, plus exemplar trace IDs at that percentile — ideal baseline candidates.
...
The MinHash columns are a general-purpose trace shape fingerprint. Beyond baseline selection:
...
- **Alert grouping** — partition failing traces by trace shape
...
- A single TraceQL query returns shape-similar baseline candidates without client-side orchestration
